...
**A green sweep is not a green CI.** Four pull-request checks are *not* in it:
...
Those are exactly the gates that catch a broken migration, prohibited release language, and unbuildable Windows or Android packages. `scripts/agent-sweep-coverage.test.mjs` fails if that list drifts.
