...
describe('media jobs', () => {
  ...
  it('refuses every media route to an unauthenticated caller', async () => {
    const provider = { transcribe: vi.fn() };
    const app = buildApp(provider);
    prisma._store.entity.push({
      ...
      type: 'MediaJob',
      userId: 'owner',
      data: { user_id: 'owner', status: 'completed', transcript: 'Private transcript' },
      createdAt: new Date(),
      updatedAt: new Date(),
    });

    const [upload, list, detail, removal] = await Promise.all([
      request(app).post('/api/media/jobs').set('Content-Type', 'text/plain').send('Transcript'),
      request(app).get('/api/media/jobs'),
      request(app).get('/api/media/jobs/someone-elses-job'),
      request(app).delete('/api/media/jobs/someone-elses-job'),
    ]);

    expect([upload.status, list.status, detail.status, removal.status]).toEqual([401, 401, 401, 401]);
    expect(provider.transcribe).not.toHaveBeenCalled();
    // Nothing new was minted, and the existing owner's transcript never leaked.
    expect(prisma._store.entity).toHaveLength(1);
    expect(JSON.stringify([upload.body, list.body, detail.body, removal.body]))
      .not.toContain('Private transcript');
  });

  it('refuses an unauthenticated caller even on a forged premium content type', async () => {
    const provider = { transcribe: vi.fn() };
    const app = buildApp(provider);
    const response = await request(app)
      .post('/api/media/jobs')
