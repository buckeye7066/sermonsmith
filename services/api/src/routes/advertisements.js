...
const publicAd = (r) => ({ id: r.id, advertiser: r.advertiser, headline: r.headline, body: r.body, creative: r.creative, seconds: r.seconds, revision: r.revision });
...
router.put('/owner/:id', owner, wrap(async (req,res) => {
  ...
  const n = await prisma.$executeRaw`UPDATE advertisements SET revision=revision+1,advertiser=${a.advertiser},headline=${a.headline},body=${a.body},creative=${a.creative},url=${a.url},seconds=${a.seconds},active=${a.active},starts_at=${a.startsAt},ends_at=${a.endsAt},image=COALESCE(${image}::bytea,image) WHERE id=${req.params.id} AND NOT removed`;
