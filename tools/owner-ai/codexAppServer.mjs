  ...
  return new Promise(resolve=>{
    ...
    function terminateChild(){
      ...
      if(platform==='win32'&&Number.isInteger(child.pid)){
        try{const killer=spawnImpl(path.join(env.SystemRoot||'C:\\Windows','System32','taskkill.exe'),['/PID',String(child.pid),'/T','/F'],{shell:false,windowsHide:true,stdio:'ignore'});killer.on?.('error',()=>{try{child.kill()}catch{}});killer.on?.('close',code=>{if(code!==0){try{child.kill()}catch{}}})}catch{try{child.kill()}catch{}}
