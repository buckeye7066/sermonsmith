/**
 * Canonical per-chapter verse counts (KJV / Protestant versification).
 *
 * Keyed by lower-cased book name to match scriptureRefs' BOOK_LOOKUP. Each
 * value is an array whose length equals the book's chapter count and whose
 * element [i] is the number of verses in chapter i+1. Used to range-check a
 * reference's verse number exactly (e.g. John 3 has 36 verses, so "John 3:37"
 * is out of range) instead of relying on the coarse 176-verse global ceiling.
 *
 * `psalm` and `song of songs` are aliases added at the bottom so both spellings
...
// Aliases — share the same array reference as their canonical spelling.
