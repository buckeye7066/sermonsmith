...
// FULL-DOMAIN SCAN — no range allowlist. Iterate EVERY Unicode scalar value
// (U+0000..U+10FFFF, excluding the surrogate block) and bucket it by the ACTUAL
// predicate `normalizeTokenChar`: a code point is a token source IFF its full
// normalized form differs from itself and is wholly token chars (checked per
// targetSet in derivedClassChars). Because the predicate IS the scan, no source can
// be omitted (unlike a range allowlist, which missed U+1D9C ᶜ→c, U+2C7D ⱽ→V,
// U+1F132 🄲→C). Decimal digits (\p{Nd}) are matched by the class directly, so
// skipped here. One-time cost at load (~150ms, NFKC-dominated) — acceptable; the
// derived class is the EXACT set { c : normalizeTokenChar(c) ⊆ tokenset }.
function scanCompatTokenSources() {
  const out = [];
  for (let cp = 0; cp <= 0x10FFFF; cp += 1) {
    if (cp >= 0xD800 && cp <= 0xDFFF) continue;
    const src = String.fromCodePoint(cp);
    if (ND_ONE_RE.test(src)) continue; // decimal digit → matched by \p{Nd} in the class
    const norm = normalizeTokenChar(src);
    if (norm && norm !== src) out.push([src, norm]);
  }
  return out;
}
// Memoize the one-time full-domain scan on globalThis so repeated module loads within
// the same process (e.g. per-file test isolation) reuse it instead of re-scanning.
const COMPAT_TOKEN_SOURCES = (globalThis.__ssCompatTokenSources || (globalThis.__ssCompatTokenSources = scanCompatTokenSources()));
// Escaped class-chars for every source whose normalized canonical ⊆ targetSet.
...
// TEST SUPPORT: for each ASCII token char, EVERY compatibility source char (from the
// FULL-DOMAIN scan) that normalizes to it — the exact reverse of the decoder's flank
// derivation, so decoder and lock share ONE complete source of truth and neither can
// miss a code point. Only single-char canonicals are exposed (a multi-char source like
// Ⅳ→"IV" is covered per-char by Ⅰ/Ⅴ-style single-char sources). The lock iterates
// EVERY source here, so it samples every discovered surface bucket the scan produces.
