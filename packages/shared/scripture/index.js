...
// ── TOKEN SURFACE DERIVED FROM THE ACTUAL NORMALIZATION (NFKC + explicit folds) ──
// The recurring r45–r48 root cause was the decoder's flank token classes replicating
// only PART of the grammar's normalization. The grammar's pass 2 applies NFKC on top
// of the explicit folds, so NFKC-only compatibility tokens (subscript digits ₄₅,
// mathematical letters 𝐈𝐕, circled digits, …) fold into grammar tokens but were NOT in
// the decoder classes → a literal escaped seam around them bypassed. DEFINITIVE FIX:
// the flank predicate IS the actual normalization the grammar applies — NFKC ∪ the
// explicit folds (archaic Roman that does NOT NFKC-fold, decimalDigitToAscii for every
// \p{Nd} script, fullwidth-Latin, superscript). A char is a NUMBER/ROMAN/ORD-SUFFIX
// token char IFF normalizeTokenChar(c) ∈ the ASCII token set. So decoder surface ≡
// grammar surface BY CONSTRUCTION: subscript / mathematical / circled / any-future
// compatibility form is inherited automatically. The precomputed regex CLASSES are
// GENERATED by scanning the Unicode compatibility ranges (BMP + SMP) and NFKC-folding
// each — derived from NFKC, not hand-listed. The decoder only NORMALIZES the seam; the
// validator still classifies the canonical token (MMMM / iiii still flagged, r31), and
// the r35/r36 position-aware superscript handling (bounded scrub) is untouched.
const clsEsc = (ch) => {
  const cp = ch.codePointAt(0);
  return cp > 0xFFFF ? `\\u{${cp.toString(16)}}` : `\\u${cp.toString(16).padStart(4, '0')}`;
};
// Apply the SAME normalization the extractor's pass 2 applies, to decide token-hood:
// NFKC, THEN the explicit folds normalizeCitationText adds on top of NFKC (archaic
// Roman that does NOT NFKC-fold; every \p{Nd} script → ASCII; fullwidth-Latin;
// superscript). Returns the ASCII canonical string (possibly multi-char, e.g. Ⅳ→"IV").
function normalizeTokenChar(ch) {
  let out = '';
  for (const c of ch.normalize('NFKC')) {
    if (ROMAN_NUMERAL_FOLD.has(c)) { out += ROMAN_NUMERAL_FOLD.get(c); continue; }
    const code = c.codePointAt(0);
    if ((code >= 0xFF21 && code <= 0xFF3A) || (code >= 0xFF41 && code <= 0xFF5A)) { out += String.fromCharCode(code - 0xFEE0); continue; }
    if (Object.prototype.hasOwnProperty.call(SUPERSCRIPT_FOLD, c)) { out += SUPERSCRIPT_FOLD[c]; continue; }
    if (ND_ONE_RE.test(c)) { out += decimalDigitToAscii(c); continue; }
    out += c;
  }
  return out;
}
// The COMPATIBILITY code-point pool: the Unicode ranges carrying compatibility
// digit/letter forms (super/subscripts, letterlike + Roman numerals, enclosed/circled,
// fullwidth, mathematical alphanumerics) ∪ the explicit fold-table keys — the token
// classes are GENERATED by NFKC-folding each, so nothing is hand-listed. \p{Nd}
// decimals and \p{L} letters are matched by those classes directly.
const COMPAT_SCAN_RANGES = [
  [0x00A0, 0x00FF], [0x02B0, 0x02FF], [0x1D2C, 0x1D6A], [0x2070, 0x209F],
  [0x2100, 0x2188], [0x2460, 0x24FF], [0x2776, 0x2793], [0xFF00, 0xFFEF],
  [0x1D400, 0x1D7FF], [0x1FBF0, 0x1FBF9],
];
const COMPAT_SOURCES = new Set([...ROMAN_NUMERAL_FOLD.keys(), ...Object.keys(SUPERSCRIPT_FOLD)]);
for (const [lo, hi] of COMPAT_SCAN_RANGES) for (let cp = lo; cp <= hi; cp += 1) COMPAT_SOURCES.add(String.fromCodePoint(cp));
// [src, NFKC-canonical] for every compatibility source that is NOT itself a plain
// decimal digit (those are matched by \p{Nd} directly).
const COMPAT_TOKEN_SOURCES = [];
for (const src of COMPAT_SOURCES) {
  if (ND_ONE_RE.test(src)) continue;
  const norm = normalizeTokenChar(src);
  if (norm && norm !== src) COMPAT_TOKEN_SOURCES.push([src, norm]);
}
// Escaped class-chars for every compat source whose NFKC canonical ⊆ targetSet.
const derivedClassChars = (targetSet) => COMPAT_TOKEN_SOURCES
  .filter(([, norm]) => [...norm].every((c) => targetSet.has(c)))
...
// TEST SUPPORT: for each ASCII token char, the compatibility source chars that
// NFKC-canonicalize to it, bucketed BY SURFACE (fullwidth / subscript / superscript /
// mathematical / romanNumeral / circled) — DERIVED by scanning the SAME compatibility
// ranges + NFKC the decoder derives from. The grammar-parity lock uses this to render
// each ASCII token in every NFKC surface, so a NEW compatibility form is covered by
// the decoder AND exercised by the lock automatically (one shared source of truth).
export function __citationFoldSurfaces() {
  const surfaceOf = (cp) => {
    if ((cp >= 0xFF10 && cp <= 0xFF19) || (cp >= 0xFF21 && cp <= 0xFF5A)) return 'fullwidth';
    if (cp >= 0x2080 && cp <= 0x2089) return 'subscript';
    if (cp === 0xB2 || cp === 0xB3 || cp === 0xB9 || (cp >= 0x2070 && cp <= 0x207F)) return 'superscript';
    if (cp >= 0x1D400 && cp <= 0x1D7FF) return 'mathematical';
    if (cp >= 0x2160 && cp <= 0x2188) return 'romanNumeral';
    if ((cp >= 0x2460 && cp <= 0x24FF) || (cp >= 0x2776 && cp <= 0x2793)) return 'circled';
    return 'other';
  };
  // ASCII target char → { surfaceName: sourceChar } for SINGLE-char NFKC canonicals.
  const m = new Map();
  const ensure = (t) => { if (!m.has(t)) m.set(t, {}); return m.get(t); };
  for (const [src, norm] of COMPAT_TOKEN_SOURCES) {
    if (norm.length !== 1) continue; // multi-char (Ⅳ→"IV") handled at token level by the lock
    const surf = surfaceOf(src.codePointAt(0));
    if (surf !== 'other' && !(surf in ensure(norm))) ensure(norm)[surf] = src;
  }
  return m;
}
