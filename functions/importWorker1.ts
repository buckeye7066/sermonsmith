...
// Worker 1: First 10 translations
const ASSIGNED_TRANSLATIONS = ['KJV', 'ASV', 'BBE', 'DARBY', 'WEB', 'YLT', 'ESV', 'NIV', 'NRSV', 'NASB'];
...
Deno.serve(async (req) => {
  ...
  console.log(`[WORKER 1] Starting import for: ${ASSIGNED_TRANSLATIONS.join(', ')}`);
...
async function importTranslations(base44, translationIds) {
  for (const translationId of translationIds) {
    console.log(`[WORKER 1] Starting ${translationId}...`);
    ...
    try {
      ...
      console.log(`[WORKER 1] ✅ ${translationId} complete: ${totalVerses} verses`);

    } catch (error) {
      console.error(`[WORKER 1] ❌ ${translationId} failed:`, error.message);
    }

    await sleep(2000);
  }

  console.log('[WORKER 1] 🎉 All assigned translations complete!');
}
