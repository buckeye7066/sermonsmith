...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`  Developer Access: ${isDeveloper ? '✅ YES (Email/Phone Match)' : '❌ NO'}`);
    ...
    try {
      ...
      if (translationsResult.status === 200 && translationsResult.data?.translations) {
        ...
        premiumTest.subtests.push({
          ...
          isDeveloper: translationsResult.data.is_developer,
        ...
        console.log(`  ✓ Developer Mode: ${translationsResult.data.is_developer ? 'ON' : 'OFF'}`);
      } else {
