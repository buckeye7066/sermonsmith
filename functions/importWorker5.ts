...
// Worker 5: Remaining translations
const ASSIGNED_TRANSLATIONS = ['DRA', 'AKJV', 'LEB', 'MOUNCE', 'WEBBE', 'WMBBE', 'WMBME', 'RV', 'CPDV', 'JUBILEE2000', 'WEBSTER'];
...
Deno.serve(async (req) => {
  ...
  console.log(`[WORKER 5] Starting import for: ${ASSIGNED_TRANSLATIONS.join(', ')}`);
...
async function importTranslations(base44, translationIds) {
  for (const translationId of translationIds) {
    console.log(`[WORKER 5] Starting ${translationId}...`);
    ...
    try {
      ...
      console.log(`[WORKER 5] ✅ ${translationId} complete: ${totalVerses} verses`);

    } catch (error) {
      console.error(`[WORKER 5] ❌ ${translationId} failed:`, error.message);
    }

    await sleep(2000);
  }

  console.log('[WORKER 5] 🎉 All assigned translations complete!');
}
