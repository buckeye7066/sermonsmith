...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('[DIRECT IMPORT] Starting direct import for user:', user.email);

    // Get all enabled translations
    const translations = await base44.asServiceRole.entities.Translation.filter({ enabled: true });
    console.log(`[DIRECT IMPORT] Found ${translations.length} translations to import`);

    // Start import in background
    importAllTranslations(base44, translations);

    return Response.json({
      success: true,
      message: `Direct import started for ${translations.length} translations`,
...
async function importAllTranslations(base44, translations) {
  for (const trans of translations) {
    console.log(`\n[DIRECT IMPORT] Starting ${trans.id}...`);
    ...
    try {
      ...
      for (const book of BIBLE_BOOKS) {
        for (let chapter = 1; chapter <= book.chapters; chapter++) {
          ...
          // Fetch verses
          const verses = await fetchChapter(trans.id, book.name, chapter);
          ...
          const records = verses.map(v => ({
            ...
            verse: v.verse,
            text: v.text,
            source_hash: `${trans.id}-${book.name}-${chapter}-${v.verse}`
          }));
          ...
          if (totalChapters % 50 === 0) {
            console.log(`[DIRECT IMPORT] ${trans.id}: ${totalChapters}/1189 chapters, ${totalVerses} verses`);
          }
      ...
      console.log(`[DIRECT IMPORT] ✅ ${trans.id} COMPLETE: ${totalVerses} verses, ${totalChapters} chapters`);

    } catch (error) {
      console.error(`[DIRECT IMPORT] ❌ ${trans.id} failed:`, error.message);
    }
  ...
  console.log('[DIRECT IMPORT] 🎉 ALL TRANSLATIONS IMPORTED!');
}
