...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('[TEST] Email match:', emailMatch);
    console.log('[TEST] Phone match:', phoneMatch);
    console.log('[TEST] Is developer:', emailMatch || phoneMatch);
    
    if (!emailMatch && !phoneMatch) {
      console.log('[TEST] ❌ Not a developer - returning 403');
      return Response.json({ 
        error: 'Admin access required',
        user_email: user.email,
        checked_against: devEmails
      }, { status: 403 });
    }
