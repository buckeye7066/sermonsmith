...
// Worker 2: Next 10 translations
const ASSIGNED_TRANSLATIONS = ['NLT', 'MSG', 'AMP', 'CEV', 'GNT', 'HCSB', 'ISV', 'NET', 'RSV', 'NKJV'];
...
Deno.serve(async (req) => {
  ...
  console.log(`[WORKER 2] Starting import for: ${ASSIGNED_TRANSLATIONS.join(', ')}`);
...
async function importTranslations(base44, translationIds) {
  for (const translationId of translationIds) {
    console.log(`[WORKER 2] Starting ${translationId}...`);
    ...
    try {
      ...
      console.log(`[WORKER 2] ✅ ${translationId} complete: ${totalVerses} verses`);

    } catch (error) {
      console.error(`[WORKER 2] ❌ ${translationId} failed:`, error.message);
    }

    await sleep(2000);
  }

  console.log('[WORKER 2] 🎉 All assigned translations complete!');
}
