...
// Worker 3: Next 10 translations
const ASSIGNED_TRANSLATIONS = ['GW', 'TLB', 'ERV', 'EXB', 'ICB', 'NIRV', 'NCV', 'PHILLIPS', 'VOICE', 'CEB'];
...
Deno.serve(async (req) => {
  ...
  console.log(`[WORKER 3] Starting import for: ${ASSIGNED_TRANSLATIONS.join(', ')}`);
...
async function importTranslations(base44, translationIds) {
  for (const translationId of translationIds) {
    console.log(`[WORKER 3] Starting ${translationId}...`);
    ...
    try {
      ...
      console.log(`[WORKER 3] ✅ ${translationId} complete: ${totalVerses} verses`);

    } catch (error) {
      console.error(`[WORKER 3] ❌ ${translationId} failed:`, error.message);
    }

    await sleep(2000);
  }

  console.log('[WORKER 3] 🎉 All assigned translations complete!');
}
