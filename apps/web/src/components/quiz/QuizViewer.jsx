...
export default function QuizViewer({ quizData, onSave, user }) {
  ...
  const handleExport = async () => {
    ...
    try {
      const { jsPDF } = await import('jspdf').catch(error => {
        console.error('Failed to load PDF library:', error);
        toast.error("Failed to load the PDF library. Please try again later.");
        throw error; // Rethrow to ensure the rest of handleExport doesn't execute.
      });
      const doc = new jsPDF();
      ...
      doc.save(`${(editedTitle || 'quiz').replace(/[^a-z0-9]+/gi, '_').replace(/^_+|_+$/g, '') || 'quiz'}.pdf`);
