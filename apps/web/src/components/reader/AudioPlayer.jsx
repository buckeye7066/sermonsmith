...
export default function AudioPlayer({ verses, book, chapter, isPremium, isOnline, currentTranslation }) {
  ...
  const handleVolumeChange = (value) => {
    setVolume(value);
    // Persist the level so toggleMute's `savedVolume` read restores it (that
    // read already existed but nothing ever wrote the key, so unmuting always
    // snapped back to 80).
    localStorage.setItem('savedVolume', JSON.stringify(value));
    // speechSynthesis is optional (SSR, hardened browsers, some WebViews), so
    // go through resolveSpeechSynthesis rather than touching window directly.
    const browserScope = typeof window === 'undefined' ? undefined : window;
    const synthesis = resolveSpeechSynthesis(browserScope);
