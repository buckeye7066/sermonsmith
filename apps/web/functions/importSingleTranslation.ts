...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!translationId) {
      return Response.json({ error: 'translationId required' }, { status: 400 });
    }
    ...
    for (const book of BIBLE_BOOKS) {
      for (let chapter = 1; chapter <= book.chapters; chapter++) {
        ...
        if (totalChapters % 100 === 0) {
          console.log(`[IMPORT] ${translationId}: ${totalChapters}/1189 chapters, ${totalVerses} verses`);
        }
    ...
    console.log(`[IMPORT] ✅ ${translationId} COMPLETE: ${totalVerses} verses, ${totalChapters} chapters (${skippedChapters} already existed)`);
    ...
    return Response.json({
      ...
      message: `${translationId} imported successfully`
    });
